"""
Simple outburst flood model: Calculate time-dependent S, Q, p_w
"""
...
Q = -S**(5/4)*(1/fR/rhow)**(1/2)*(1/np.pi)**(1/4)*dphids*np.abs(dphids)**(-0.5)

# Now we have everything we need: S, Q, p_w
# Take one timestep
# Calculate: dSdt
mi = Q/Lf * (gamma - 1) * dpwds

dSdt = mi/rhoi - 2*S*A*((p_i - pw)/n)**n

S = S + dt*dSdt

# Now solve for nonsteady Q
D = D_upwind + (1/rhoi - 1/rhow)/Lf*(gamma - 1)*np.diag(dphids)
y = np.vstack(2*S*A*( (p_i - pw)/n)**n)
y[0] = y[0] + Q0/dx # Boundary condition on Q[0]
